...
dishRouter.route('/:dishId/comments').get((req, res, next)=>{
	...
	.then((dish)=> {
		if(dish!=null){
			console.log("no of comments: "+dish.comments.length);
			
			for(var i=(dish.comments.length-1); i>=0; ){
				console.log("comment: "+dish.comments[i]);
				dish.comments.id(dish.comments[i]._id).remove();
				console.log("removed comment at "+i);
				i--;
			//	dish.comments.get(i).remove();
			}
			dish.save().then((dish)=>{
				res.statusCode= 200;
				res.setHeader('Content-Type','application/json');
...
dishRouter.route("/:dishId/comments/:commentId").get((req, res, next)=>{
	...
	.then((dish)=>{
		...
		else if(dish==null)
		{
			...
			err = new Error('Comment '+req.params.dishId+'/comments/'+req.params.commentId+' not found');
			err.status = 404;
		}
